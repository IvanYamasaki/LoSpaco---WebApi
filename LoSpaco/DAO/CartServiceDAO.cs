...
namespace  LoSpacoWebAPi.DAO {
    public class CartServiceDAO {
        public static List<CartService> GetList() {
            ...
            Database.ReaderRows(Database.ReturnProcedure("sp_SelectCart", Security.Context.GetUser()), row => list.Add(new CartService((ushort)row[0], (string)row[1], (byte)row[2], (decimal)row[4], (byte[])row[5])));
            return list;
        }

        public static CartService GetByName(string name) {
            string query = $"select ItemId, ItemName, ItemQnt, itemPrice, itemImage from vw_cart where ItemName='{name}' and LoginEmail = '{Security.Context.GetUser()}'";
